import {FormHelperText, Stack, TextField, Typography,Box, useTheme,IconButton,
  InputAdornment, useMediaQuery} from '@mui/material'
...
export const Signup = () => {
  ...
  return (
    ...
    // width={'100vw'} height={'100vh'} flexDirection={'row'} sx={{overflowY:"hidden"}}
    >
      ...
      {/* {
        ...
        />
              <Stack flexDirection={'row'} justifyContent={'center'} alignItems={'center'}>
                  <Stack rowGap={'.4rem'}>
                    <Typography variant='h5' sx={{color: "white",wordBreak:"break-word",textAlign:"center"}} fontWeight={500}>Thekkady Spices</Typography>
                    <Typography alignSelf={'center'} color="white" variant='body2'>Please create New Account</Typography>
                  </Stack>
                   ...
                   sx={{
                    ...
                    input: {
                     ...
                     sx={{
                      ...
                      <motion.div>
                        <TextField type={showPassword ? "text" : "password"} fullWidth {...register("password",{required:"Password is required",pattern:{value:/^(?=.*\d)(?=.*[a-z])(?=.*[A-Z])(?=.*[a-zA-Z]).{8,}$/gm,message:`at least 8 characters, must contain at least 1 uppercase letter, 1 lowercase letter, and 1 number, Can contain special characters`}})} placeholder='Password'
                          InputProps={{
                            endAdornment: (
                ...
                />
                      ...
                      <motion.div>
                        <TextField type={showcPassword ? "text" : "password"} fullWidth {...register("confirmPassword",{required:"Confirm Password is required",validate:(value,fromValues)=>value===fromValues.password || "Passwords doesn't match"})} placeholder='Confirm Password'
                          InputProps={{
                            endAdornment: (
                              <InputAdornment position="end">
                                <IconButton onClick={() => setShowcPassword(!showcPassword)} edge="end">
                                  {showcPassword ? <VisibilityOff sx={{ color: "black" }} /> : <Visibility sx={{ color: "black" }} />}
                                </IconButton>
                              </InputAdornment>
                            ),
                          }}
            ...
            "&:hover": { backgroundColor: "#089b08" },
          }} loading={status==='pending'} type='submit' variant='contained'>Signup</LoadingButton>
                    </motion.div>
                    ...
                    <Stack flexDirection={'row'} justifyContent={'space-between'} alignItems={'center'} flexWrap={'wrap-reverse'}>
                        <MotionConfig whileHover={{x:2}} whileTap={{scale:1.050}}>
                            ...
                            <motion.div>
                                <Typography sx={{textDecoration:"none",color:"white"}} to={'/login'} component={Link}>Already a member? <span style={{color:"#089b08",fontWeight:"500"}}>Login</span></Typography>
                            </motion.div>
                        </MotionConfig>
                    </Stack>
