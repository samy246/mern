...
exports.signup=async(req,res)=>{
    try {
        ...
        const cookieName = createdUser.isAdmin ? "adminToken" : "userToken";
...
exports.login=async(req,res)=>{
    try {
        ...
        if(existingUser && (await bcrypt.compare(req.body.password,existingUser.password))){
            ...
            // generating jwt token
            const token=generateToken(secureInfo)
            const cookieName = existingUser.isAdmin ? "adminToken" : "userToken";
            // sending jwt token in the response cookies
            res.cookie(cookieName,token,{
                sameSite:process.env.PRODUCTION==='true'?"None":'Lax',
                maxAge:new Date(Date.now() + (parseInt(process.env.COOKIE_EXPIRATION_DAYS * 24 * 60 * 60 * 1000))),
            ...
            // await sendMail("tharnish1998@gmail.com,letsgametech@gmail.com",`Admin duraisam632@gmail.com logIn `,`This will be testing purpose`)
        ...
        // await sendMail("tharnish1998@gmail.com",`Admin duraisam632@gmail.com`)
    }
}
...
exports.resendOtp=async(req,res)=>{
    try {
        ...
        await sendMail(existingUser.email,`OTP Verification for Your Thekkady Spices New Account`,`Your One-Time Password (OTP) for account verification is: <b>${otp}</b>.</br>Do not share this OTP with anyone for security reasons`)
...
exports.forgotPassword=async(req,res)=>{
    ...
    try {
        ...
        await sendMail(isExistingUser.email,'Password Reset Link for Thekkady Spices Account',`<p>Dear ${isExistingUser.name},

        We received a request to reset the password for your Thekkady Spices account. If you initiated this request, please use the following link to reset your password:</p>
